...
ThreshModFixedAlphaResp <- function(dat,commonslope ,indicator=indicator, der=der,lin =lin, start, startind,
  ...
  if (commonslope == "var"){
    ...
    if (der == "der"){
      ...
      #LoglikNAlpha(par ,dat,I,indicator, lin=lin, penalpha=0)
  ...
  BIC <-  2*fit$value +2*log(P)*length(par)
...
deltalinNVdensDiscrAlpha <-function(y,iteml,slope , theta,lin=lin){
  ...
  for (i in 1:length(y)){
    ...
    if(y[i] > 0){
      ...
      valr <- diffunctlin(iteml[1],iteml[2],y[i])
      valr1 <- diffunctlin(iteml[1],iteml[2],y[i]-1)
...
##### GH function derivative   continuous, now with discrete
  ...
  if(indicator=="D"){
    ...
    #### general Oct 22
      ...
      if (datitem[comp]>0){
        ...
        #arg1 <- slope[comp]*theta - diffunct(iteml[1],iteml[2],datitem[comp]-1)   
        #arg2 <- slope[comp]*theta - iteml[1]-iteml[2]*log(1+datitem[comp])
        #arg2 <- slope[comp]*theta - diffunct(iteml[1],iteml[2],datitem[comp])
        #prod <- -dnorm(arg1,0,1)+dnorm(arg2,0,1)
        #prod <- -respfctd(arg1)+respfctd(arg2)
        
        ##new
        slopel<-slope[comp]
        prod <- -deltalinNVdensDiscr(datitem[comp]-1,iteml,slopel , theta, lin =lin)+
          deltalinNVdensDiscr(datitem[comp],iteml,slopel , theta, lin =lin)}
        ...
        if (datitem[comp]<=0){
          ...
          ##new
          slopel<-slope[comp]
          prod <- deltalinNVdensDiscr(datitem[comp],iteml,slopel , theta, lin =lin)}
...
prodfctderiv2Alpha <-function(theta,datitem=datitem,I=I,parmatr=parmatr,slope=slope,indicator=indicator, lin=lin,
  ...
  ###### 2 for slopes
  ### comp is item 
  ### only for indicator C, now also D
...
prodfctderivalphaAlpha <-function(theta,datitem=datitem,I=I,parmatr=parmatr,slope=slope,indicator=indicator, lin=lin,
  ...
  if(indicator=="D"){
      ...
      if (datitem[comp]>0){
        ...
        prod <- theta*respfctd(arg1)-theta*respfctd(arg2)}
      
      if (datitem[comp]<=0){
        #arg2 <- slope[comp]*theta - iteml[1]-iteml[2]*log(1+datitem[comp])
        arg2 <- slope[comp]*theta - diffunct(iteml[1],iteml[2],datitem[comp])
        #prod <- -theta*dnorm(arg2,0,1)
        prod <--theta*respfctd(arg2)}  
...
#### wohl unnötig, nicht modifiziert
